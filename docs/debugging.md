...
*Nota:* O valgrind neste momento não tem suporte completo para Macs. Vejam a FAQ sobre como instalar a aplicação leaks (o equivalent do valgrind para Mac). Outras alternativas são utilizar uma máquina virtual com ubuntu, os computadores dos laboratórios, ou o nó sigma (`ssh ist1xxxxx@sigma.tecnico.ulisboa.pt` com password do fénix).
