...
* As linhas não deverão exceder um limite de 80 caracteres: `wc -L`.
...
* Deve utilizar o formato Doxygen para produzir os comentários do código. Inicialmente, para gerar o ficheiro de configuração __Doxyfile__, execute *doxygen -g*. Pode alterar os parâmetros em __Doxyfile__, nomeadamente __PROJECT_NAME__ e __EXTRACT_ALL__. Para gerar a documentação executar *doxygen Doxyfile*. A documentação em __HTML__ encontra-se em *html/index.html*.
* O código deve ser adequadamente comentado. Comentários em excesso devem ser evitados. Por exemplo, não é necessário comentar sobre uma variável que representa um contador de um ciclo.
...
* Recomenda-se usar a ferramenta [lizard](http://www.lizard.ws/), _pip install lizard_, para calcular a complexidade ciclométrica e o número de linhas de código. A ferramenta não deve emitir avisos ao executar o comando seguinte.
...
*Nota:* O valgrind neste momento não tem suporte para Macs. Vejam a FAQ na página da disciplina sobre como instalar a aplicação leaks para Mac. Outras alternativas são utilizar uma máquina virtual com ubuntu, os computadores dos laboratórios, ou o nó sigma (`ssh ist1xxxxx@sigma.tecnico.ulisboa.pt` com password do fénix).
